...
ValidateUnknownFlags(args, inlinePrompt, modelOverride);
...
ChatCompletionOptions BuildOptions()
{
    var opts = new ChatCompletionOptions();
    foreach (var t in toolRegistry.ToChatTools()) opts.Tools.Add(t);
    foreach (var t in McpManager.ToChatTools())   opts.Tools.Add(t);
    return opts;
}

var slashHandler = new SlashCommandHandler(
    config, toolRegistry, client, BuildClient, BuildSystemPrompt,
    async (c, o, m, u) => await RunTurnAsync(c, o, m, u));

// ── Dispatch: one-shot or interactive REPL ────────────────────────────────────
...
async Task RunReplAsync()
{
    ...
    while (true)
    {
        ...
        {
            // /stats is handled here because it owns session counters
        ...
        foreach (var (acc, toolCall) in accumulators.Values.OrderBy(a => a.Index).Zip(toolCalls))
        {
            ...
            try
            {
                toolResult = acc.Name.StartsWith("mcp__")
                    ...
                    : await toolRegistry.ExecuteAsync(acc.Name, acc.Arguments);
            }
            catch (Exception ex)
            {
...
void PrintStats(List<ChatMessage> messages)
{
...
static void ValidateUnknownFlags(string[] args, string? inlinePrompt, string? modelOverride)
{
